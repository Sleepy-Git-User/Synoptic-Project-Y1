...
app.get('/p1', function(req, res){
    res.sendFile(page_directory + 'p1.html');
});
...
app.post('/submit', async function(req, res){
    ...
    if (req.body.fname.length <= 0 || req.body.lname.length <= 0 || req.body.email.length <= 0){
        //Resends webpage if theres an error
        res.sendFile(page_directory + '-');
    }
    else{ //Submission was populated correctly
        //Add submission to Submissions.json
        fs.appendFile("Submissions.json",JSON.stringify(req.body)+"\n",(err)=>{
            if (err){
                console.log(err);
        ...
        await transporter.sendMail({
            ...
            html: "<h1>"+ req.body.fname + " " + req.body.lname + " thank you for signing up! We will get back to you shortly!</h1>"
        });
